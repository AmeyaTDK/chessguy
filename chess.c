...
struct piece *source_finder(){
    ...
    if(cb[rank][file].p != NULL && cb[rank][file].p->color == turn_flag){ //Own piece on dst sqr
        printf("Can't capture own pieces, try again\n");                   
        return NULL;
    }
    ...
    if(cmet.type == P){
        ...
        if(cmet.capture == false){
            if(cb[rank][file].p != NULL){
                printf("Invalid move,Can't move to a preoccupied square\n");
                return NULL;
            }
            if(turn_flag == 1){
                ...
                printf("Invalid move(Piece could not be founnd), try again\n");
                return NULL;
            }
        }
            ...
            if(cb[rank][file].p->color == turn_flag){
                printf("Invalid move,Can't capture own pieces\n");
                return NULL;
            }
            if(turn_flag == 1){
                if(cb[rank-1][file-1].p != NULL && cb[rank-1][file-1].p->type == P && file-1 == (cmet.src_file-'a')) 
                    return (cb[rank-1][file-1]).p;
                else if(cb[rank-1][file+1].p != NULL && cb[rank-1][file+1].p->type == P && file+1 == (cmet.src_file-'a'))
                    return (cb[rank-1][file+1]).p;
            }
            else if(turn_flag == -1){
                if(cb[rank+1][file-1].p != NULL && cb[rank+1][file-1].p->type == P && file-1 == (cmet.src_file-'a'))
                ...
                printf("Invalid move(Piece coud not be found), try again\n");
                return NULL;
            }
        }        
    }
